...
- 데이터베이스에 주소 테이블에는 userId가 있는데 데이터를 확인해보니 유저 테이블에는 userId가 잘 들어가있는데 주소 테이블에는 userId가 계속 0이었다. 그래서 출력으로 하나씩 추적해서 찾아봤더니 머릿속으로는 이해가 되질 않아 이 방법 저 방법 써보다가 User 객체를 전역 변수로 빼니까 됐다!
