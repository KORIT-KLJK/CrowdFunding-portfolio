...
- 회원가입할 때, 이메일 중복확인의 에러 메시지와 가입하기를 눌렀을 때의 이메일 에러 메시지가 중복이 되어 같이 올라오는 문제가 생김
  
	- 둘의 상태를 다르게 준 후에 중복확인을 보낼 때는 가입 요청의 이메일 에러메시지를 비워두고, 
	가입하기를 누를 때는 중복확인 요청의 이메일 에러메시지를 비워두니까 해결이 됐다.

</br>

- 회원가입할 때, 주소 포함 모든 정보를 함께 보내는데 우편번호 검색으로 주소가 들어오면 그 전 정보들이 초기화가 돼서 주소만 넘어오는 문제가 생김

	- 주소에 관한 정보들을 담은 상태를 새로 만들어주고 요청으로 주소 입력 전 정보와 주소를 담은 정보를 따로 담아서 보내니까 해결이 됐다.
 
 </br>

- 펀딩 상세 페이지에서 리워드에서 많은 문제들이 생겼다. (해당 리워드가 따로 들어오지 않고, 한 이름으로만 들어오는 문제, 개수를 추가할 때 금액이 같이 올라가는 문제, 개수를 추가할 때 금액이 2배 이상씩 올라가는 문제)

	- 앞의 두 문제의 관해서는 요청으로 받은 리워드 정보에서 또 새로운 배열을 만들어줘서 데이터를 처리하니까 가능했다. 마지막 문제는 chat gpt 활용..

</br>

- 관리자 등록 페이지에서 기부와 펀딩을 한 번에 같이 등록을 하는데, 기부에서 펀딩 데이터가 들어가기도 하고, 
펀딩에서 기부 데이터가 들어가기도 하는 문제들이 있었다.

	- 코드가 좀 복잡하긴 하지만 카테고리가 기부일 때 펀딩의 모든 상태를 초기화하고 데이터를 새로 넣고, 펀딩 때도 마찬가지로
기부의 모든 상태를 초기화해서 데이터를 새로 넣었다.

</br>

개선점

- 급한 마음에 그냥 넘어갔지만, 어차피 같은 회원가입 이메일이니까 상태를 다르게 주지 않더라도 하나의 상태로도 해결할 수 있는 방법이 없을까 고민을 해볼 것이다.

- 주소도 같이 한 번에 담아서 보낼 수 있는 방법에 대해서 계속 고민해봤지만 해결하지 못했다. 한 번에 담아서 같이 보내는 방법에 대해 고민해볼 것이다.

- 실력을 키워야겠다는 생각이 들었다..

- 하고 나서 느낀 것이 애초에 하나에 다 만들지 말고 기부나 펀딩 메인 페이지에 들어갔을 때, 해당 카테고리의 등록 페이지를 
하나 만들어서 url을 이동 시킨 다음에 거기서 요청 작업을 수행했더라면 데이터가 이렇게 꼬일 일은 없지 않았을까 하고 아쉬웠다.

- 최종적으로 전체적인 느낀점을 얘기해보자면,

1. 당장 눈 앞에 있는 것들을 처리하다 보니 나중에 정리할 것들이 산더미였다. 처음부터 전체적인 구조를 이해하고, 뒤를 생각하며 코드를 짜는 것이 효율적인 것 같다고 깨달았다.

2. 1번의 연장선이지만, 앞서 말한 문제들 포함 DB에서 처리할 수 있으면 좋을 거 같았던 것들이 꽤 많았다. 그래서 거의 이번 프로젝트에서 70%는 리액트에 시간을 써버렸다. 내 실력이 많이 부족하단 걸 깨달았고, DB를 잘해야 한다는 것을 느끼게 되었다. 

---

**팀 프로젝트에 관한 개인적인 느낀점**

- 팀 프로젝트를 시작한 지 일주일 뒤에 팀원 한 분께서 취직을 하셨는데, 그로 인해 원래 계획했던 결과물을 만들 수 없게 되었다. 하지만 그 덕분에 내가 할 수 있는 것들이 많아져 전보다 많은 성장을 할 수 있게 되어서 오히려 만족스러운 프로젝트가 되었다.

- 팀원들을 도와주고 싶고, 내 할 일도 잘하고 싶었지만 내 실력이 아직 부족하여 내 것부터 챙기느라 팀원들을 많이 도와주지 못해 아쉬웠다. 실력을 더욱 키워서 팀원분들께 도움이 되면서도 내가 하는 일도 수월하게 하는 수준까지 도달하고 싶다는 것을 느끼게 되었다.

- 나는 누군가를 이끄는 것을 좋아하고, 팀장으로서의 그 팀의 무게를 짊어가는 것도 좋아한다. 그래서 이번 프로젝트를 시작하기 전에 팀장을 자청했다. 프로젝트를 하면서도, 끝나고 나서도 이게 맞는 걸까? 또 나의 이런 생각이 맞는 걸까? 하고 스스로를 돌아보게 되었는데, 이는 틀리지 않았다는 나 자신에 대한 확신과 또 스스로를 더 알게 되었다.
